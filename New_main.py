...
from metrics.losses import ssp_loss, Mixed_Loss, New_Mixed_Loss
...
def get_argparser():
    ...
    # Datset Options
    parser.add_argument("--data_root", type=str, default='/seu_share/home/wkyang/datasets/CamVid/CamVid',
                        help="path to Dataset")
    parser.add_argument("--dataset", type=str, default='camvids',
...
def get_dataset(opts):
    ...
    if opts.dataset == 'camvids':
        ...
        train_dst = camvids.CamvidSegmentation(opts.data_root, image_set='train', transform=train_transform, num_copys=opts.num_copys)
        val_dst = camvids.CamvidSegmentation(opts.data_root, image_set='val', transform=val_transform)
    ...
    if opts.dataset == 'cityscapes':
        ...
        train_dst = Cityscapes(root=opts.data_root,
                               split='train', transform=train_transform, num_copys=ops.num_copys)
        print(
            "------------------------now copy: {:}----------------------------------".format(opts.num_copys))
...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for i, (images, labels) in (enumerate(loader)):
            ...
            preds = outputs['seg'].detach().max(dim=1)[1].cpu().numpy()
...
def main():
    ...
    while cur_itrs < opts.total_itrs:
        ...
        for sample in train_loader:
            ...
            #           if cur_itrs > opts.total_itrs // 2:

            mse, ce_1_2, ce_2_1, sym_ce, ce, ex_ce, Cosine, cos_1, cos_2 = Criterion(outputs, overlap, flips, labels)
            #loss = ce #+ sym_ce #+ 0.25 * Cosine
            ...
            ce_1_2 = cos_1.detach().cpu().numpy().item()
            ce_2_1 = cos_2.detach().cpu().numpy().item()
            sym_ce = sym_ce.detach().cpu().numpy().item()
            ce = ce.detach().cpu().numpy().item()
            ex_ce = ex_ce.detach().cpu().numpy().item()
            cosine = Cosine.detach().cpu().numpy().item()
            ...
            if cur_itrs % 10 == 0:
                ...
                total_mse, total_ce_1_2, total_ce_2_1, total_ce = total_mse / 10, total_ce_1_2 / 10, total_ce_2_1 / 10, total_ce / 10
                total_sym_ce /= 10
                total_exce = total_ce / 10
                total_cosine = total_cosine / 10

                
                print("Epoch %d, Itrs %d/%d, Loss=%f, cos_1=%f, cos_2=%f,  total_cosine=%f, sym_ce=%f, ce=%f, ex_ce=%f" %
                      (cur_epochs, cur_itrs, opts.total_itrs, interval_loss, total_ce_1_2, total_ce_2_1,  total_cosine, total_sym_ce, total_ce, total_exce))
